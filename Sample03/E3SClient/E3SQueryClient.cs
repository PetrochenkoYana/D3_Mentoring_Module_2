...
{	
	...
	public	virtual IEnumerable<T> SearchFTS<T>(string query, int start = 0, int limit = 0) where T : E3SEntity
		{
	...
	public	virtual IEnumerable SearchFTS(Type type, string query, int start = 0, int limit = 0)
		{
			HttpClient client = CreateClient();
			var requestGenerator = new FTSRequestGenerator(BaseAddress);
